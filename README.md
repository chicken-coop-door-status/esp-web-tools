...
`script/update-manifests.js` rewrites each manifest’s `parts[].path` so they always point at the newest release under `s3://ota-charlies-farm/egg/`. The script expects the AWS CLI to be available and uses the bucket/prefix/region from the following environment variables (defaults shown):

| Variable | Default | Purpose |
| --- | --- | --- |
| `OTA_BUCKET` | `ota-charlies-farm` | S3 bucket name |
| `OTA_PREFIX` | `egg/` | Prefix that contains per-release subdirectories |
