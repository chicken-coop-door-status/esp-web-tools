# AGENTS instructions for Mother Hen Egg ESP Web Tools fork

## High-level purpose
- This fork ships a pared-down ESP Web Tools landing page tailored to Mother Hen Egg field recovery. Focus on keeping the experience deliberate (bold visuals, clear checklist, two manifest paths) rather than reverting to the upstream marketing page.
...
## Repository layout
- `index.html` now drives the entire field recovery experience, loads `config/eggs.json`, and wires up the `esp-web-install-button` component. Keep any future UI adjustments here unless we add new build tooling.
- `config/eggs.json` defines the device metadata, the checklist items, and the manifest variations. Add new manifests by extending this file; the UI is data-driven.
- `static/manifests/<device>/...` holds the actual manifest JSON files. Each manifest should honor the egg partition layout (OTA table at `0x19000`) and point to our S3-hosted binaries.
